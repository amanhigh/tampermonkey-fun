...
describe('TradeRiskPlugin', () => {
  ...
  describe('run', () => {
    ...
    it('returns empty array when risk matches full limit (6400)', async () => {
      ...
      const buyOrder = new Order('HDFC', 400, OrderType.SINGLE, 'buy1', [100]);
      const ocoOrder = new Order('HDFC', 400, OrderType.TWO_LEG, 'oco1', [84, 120]);
    ...
    it('returns empty array when risk matches half limit (3200)', async () => {
      ...
      const buyOrder = new Order('TCS', 200, OrderType.SINGLE, 'buy1', [100]);
      const ocoOrder = new Order('TCS', 200, OrderType.TWO_LEG, 'oco1', [84, 120]);
    ...
    it('allows ±1% tolerance on risk', async () => {
      ...
      const buyOrder = new Order('INFY', 398, OrderType.SINGLE, 'buy1', [100]);
      const ocoOrder = new Order('INFY', 398, OrderType.TWO_LEG, 'oco1', [84, 120]);
    ...
    it('emits FAIL for non-standard risk multiple', async () => {
      ...
      const buyOrder = new Order('SBIN', 300, OrderType.SINGLE, 'buy1', [100]);
      const ocoOrder = new Order('SBIN', 300, OrderType.TWO_LEG, 'oco1', [90, 120]);
    ...
    it('skips two-leg orders without corresponding single-leg buy order', async () => {
      ...
      const ocoOrder = new Order('LT', 100, OrderType.TWO_LEG, 'oco1', [90, 120]);
    ...
    it('skips single-leg orders (only checks two-leg OCO)', async () => {
      const event = new GttRefreshEvent();
      const buyOrder = new Order('ITC', 100, OrderType.SINGLE, 'buy1', [200]);
    ...
    it('handles multiple tickers with mixed compliance', async () => {
      ...
      const hdfcBuy = new Order('HDFC', 400, OrderType.SINGLE, 'buy1', [100]);
      const hdfcOco = new Order('HDFC', 400, OrderType.TWO_LEG, 'oco1', [84, 120]);
      // SBIN: invalid risk (3000)
      const sbinBuy = new Order('SBIN', 300, OrderType.SINGLE, 'buy2', [100]);
      const sbinOco = new Order('SBIN', 300, OrderType.TWO_LEG, 'oco2', [90, 120]);
      event.orders = {
