...
/** Interaction **/
function SmartPrompt(reasons, overrides = [], callback) {
    ...
    if (!modal) {
        ...
        const getOverride = () => selectedOverride;

        // Add Reasons
        reasons.forEach((reason, index) => {
            const button = createSmartButton(reason, `smart-button-${index}`, modal, callback, getOverride);
            modal.appendChild(button);
        });

        // Add Overrides
        overrides.forEach((override, index) => {
            const radioButton = createSmartRadioButton(override, `smart-radio-${index}`, modal, (value) => {
                selectedOverride = value;
            });
            modal.appendChild(radioButton);
        });

        // Add Text Box and Cancel
        const textBox = createSmartTextBox('smart-text', modal, callback);
        modal.appendChild(textBox);

        const cancelButton = createSmartButton('Cancel', 'smart-cancel', modal, callback, getOverride);
        modal.appendChild(cancelButton);
    }

    resetModal(modal, overrides);
 ...
 */
function createSmartButton(text, id, modal, callback, getOverride) {
...
function createSmartRadioButton(text, id, modal, onChange) {
...
/**
 * Resets the modal by clearing the text input and unchecking radio buttons based on the provided overrides.
 *
 * @param {Element} modal - The modal element to reset.
 * @param {Array} overrides - An array of overrides to determine which radio buttons to uncheck.
 * @return {void} 
 */
function resetModal(modal, overrides) {
    document.getElementById('smart-text').value = '';
    overrides.forEach((_, index) => {
        document.getElementById(`smart-radio-${index}`).checked = false;
    });
}