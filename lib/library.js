...
/** Interaction **/
function SmartPrompt(reasons) {
    return new Promise((resolve) => {
        const modal = createSmartModal();
        document.body.appendChild(modal);

        reasons.forEach((reason) => {
            const button = createSmartButton(reason, resolve, modal);
            modal.appendChild(button);
        });

        const textBox = createSmartTextBox(resolve, modal);
        modal.appendChild(textBox);

        const cancelButton = createSmartButton('Cancel', resolve, modal);
...
function createSmartButton(text, resolve, modal) {
...
function createSmartTextBox(resolve, modal) {
