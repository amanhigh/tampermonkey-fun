...
function SmartPrompt(reasons, overrides = [], callback) {
    ...
    if (!modal) {
        ...
        reasons.forEach((reason, index) => {
            const button = createSmartButton(reason, `smart-button-${index}`, modal, callback);
            modal.appendChild(button);
        });

        // Add Overrides
        overrides.forEach((override, index) => {
            const radioButton = createSmartRadioButton(override, `smart-radio-${index}`, modal);
        ...
        const cancelButton = createSmartButton('Cancel', 'smart-cancel', modal, callback);
