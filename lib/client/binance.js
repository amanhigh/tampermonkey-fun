...
class BinanceOrder {
    ...
    getQuantity(sl, entry) {
        ...
        let computedQuantity = (maxRisk / risk).toFixed(3);

        //Confirm Quantity
        let approvedQuantity = prompt(`SafeSL: ${safeSl}, Risk: ${risk}.\nPlease confirm Quantity`, computedQuantity);
        return approvedQuantity != null ? approvedQuantity : computedQuantity;
    }
    ...
    print(order) {
        message(`Side: ${order.side}  Type: ${order.type} Qty: ${order.quantity}
        Stop: ${order.stopPrice} Price: ${order.price}`.fontcolor('Yellow'));
        //console.log(order);
    }
}
