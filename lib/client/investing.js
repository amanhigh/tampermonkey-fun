 ...
 * @returns {Promise<Object>} - Promise resolving to the symbol data
 */
            ...
            onload: function (response) {
                if (response.status >= 200 && response.status < 400) {
                    ...
                    if (r.All.length > 0) {
                        ...
                        resolve({ name: top.name, pairId: top.pair_ID, exchange: top.exchange_name_short });
                    } else {
 ...
 */
function manageSymbolCache(symb, data = null) {
    ...
    if (data) {
        ...
        message(`Mapped ${symb} -> ${data.name} (${data.exchange})`.fontcolor('yellow'));
        return data;
    } else {
        // Retrieve from cache
        return m[symb] || null;
...
function searchSymbol(symb, callback, exchange = "") {
    let cachedData = manageSymbolCache(symb);

    if (cachedData && exchange === "") {
        callback(cachedData);
    } else {
        fetchSymbolData(symb, exchange)
            .then(data => {
                let cacheData = manageSymbolCache(symb, data);
                callback(cacheData);
            })
            .catch(error => {
                message(error.message.fontcolor('red'));
            });
    }
}
