...
function paintAlertFeed(tickers) {
    ...
    let watchNames = getInvestingNames(tickers.watch);
    let recentNames = getInvestingNames(tickers.recent);

    $("div.alertNotifData > a").each((i, e) => {
        //Check if Alert Name exists in Watch List Names;
        if (watchNames.includes(trimInvestingName(e.innerHTML))) {
            $(e).css('color', 'orangered');
        } else if (recentNames.includes(trimInvestingName(e.innerHTML))) {
            $(e).css('color', 'green');
        } else {
            $(e).css('color', 'white');
        }
