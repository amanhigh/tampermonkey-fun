...
function paintAlertFeed(tickers) {
    ...
    let watchNames = mapInvestingNames(tickers.watch);
    let recentNames = mapInvestingNames(tickers.recent);

    $("div.alertNotifData > a").each((i, e) => {
        //Check if Alert Name exists in Watch List Names;
        if (watchNames.has(trimInvestingName(e.innerHTML))) {
            $(e).css('color', 'orangered');
        } else if (recentNames.has(trimInvestingName(e.innerHTML))) {
            $(e).css('color', 'green');
        } else {
            $(e).css('color', 'white');
        }
