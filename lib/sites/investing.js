...
function paintAlertFeed(tickers, warn = false) {
    ...
    $("div.alertNotifData > a").each((i, e) => {
        ...
        // FIXME: How to after Ticker Extract Change ?
        if (warn && !tvTicker) {
...
function mapAlertToTvTicker(alertName) {
    ...
    let investingTicker = extractTicker(alertName);

    // Map to TV Ticker
    return investingToTVTickerMap[investingTicker];
}

function extractTicker(inputString) {
...
function handleAlertClick(e) {
    ...
    let investingTicker = extractTicker(name);

    // Default Event with Ticker to Open It
    let alertClickEvt = new AlertClicked(tvTicker, null);
