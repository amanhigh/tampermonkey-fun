...
function paintAlertFeed(tickers, warn = false) {
    ...
    let trimmedNameMap = buildNameMap();

    $("div.alertNotifData > a").each((i, e) => {
        let alertName = e.innerHTML;
        let ticker = getTickerFromAlertName(trimmedNameMap, alertName);
...
function getTickerFromAlertName(trimmedNameMap, alertName) {
    ...
    if (!ticker) {
        ticker = dataSiloInvesting.alertNameMap[alertName];
    }
    return ticker;
}
