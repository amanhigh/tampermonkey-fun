...
function removeFromWatchList() {
    ...
    message(`Removing from WatchList: ${removeTicker}`, 5000)
...
function alertSummary(alertData) {
    ...
    let ltp = readLtp();
    let $altz = $(`#${altzId}`);
    $altz.empty(); //Reset Old Buttons

    //Add GTT Buttons
    let m = GM_getValue(gttMapStore, {});
    let orders = m[getTicker()];
    if (orders) {
        orders.forEach((order) => {
            let shortType = order.type.contains('BUY') ? "B" : "SL";
            buildButton("", `${shortType}-${order.qty}`.fontcolor('blue'), () => {
            }).data('order-id', order.id).appendTo($altz);
        })

    }

    // Add Alert Buttons
    message(`Alerts for: ${alertData.name}`.fontcolor('skyblue'))
    if (alrts) {
        alrts.sort(((a, b) => {
