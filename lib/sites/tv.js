...
function toggleSymbols(color, index) {
    ...
    let lineSelector = `.listContainer-1OhjZIMS > div > div`;
    let hasSelector = `${lineSelector}:has(${colorSelector})`;
    let notSelector = `${lineSelector}:not(:has(${colorSelector}))`;

    //Remove Absolute Position Selector to Collapse List
    ...
    switch (index) {
        ...
        case 3:
            ...
            $(lineSelector).show();
            $(lineSelector).css('position', 'absolute') //Reverts to Default TV Config
            break;
        default:
