...
function toggleSymbols(colorIndex) {
    let symbolSelector = `span.symbolNameText-2EYOR9jS[style*='color: "${colorList[colorIndex]}"']`
    let hiddenSelector = symbolSelector + ":hidden";
    let parentSelector = `div.wrap-ZwpHWy6f:has(${symbolSelector})`;

    //If Hidden Elements exist, Show Else Hide
...
function paintTVScreener() {
    ...
    paint(screenerSymbolSelector, getWatchListTickers(false), colorList[5]);
...
function watchSummary() {
    let msg = ""
    let orders = OrderSet.getOrders();
    let $watchSummary = $(`#${summaryId}`);

    for (let i = 0; i < 5; i++) {
        buildLabel(orders[i].length.toString()+'|',colorList[i]).appendTo($watchSummary)
    }
    ...
    for (const ticker of watchTickers) {
        if (indexSymbols.includes(ticker) || isCompositeTicker(ticker)) {
            indexCount++;
        }
    }

    // Total Count without Index
    buildLabel((watchTickers.length - indexCount).toString() +'|',colorList[5]).appendTo($watchSummary)

    //Add Index Count @ End
    buildLabel(indexCount.toString() +'|',colorList[6]).appendTo($watchSummary)
}
...
function alertSummary(alertData) {
    ...
    buildButton("aman-refresh", "R", onReferesh).css("background-color", "black").appendTo($altz);
}
