...
const tickerMapStore = "tickerMapStore";
...
function getWatchListNames() {
    ...
    return getWatchListTickers(false).map((i, t) => {let info=m[t];return info ? info.name : t;}).toArray();
}
...
function getNameMap(refresh = false) {
    if (!nameTickerMapMem || refresh) {
        nameTickerMapMem = {}; //Reset Name Map
        let m = GM_getValue(pairMapStore, {}) //Load Pair Map

        //Map Name to Ticker
        Object.keys(m).forEach((key) => {
            nameTickerMapMem[m[key].name] = key;
            console.log(m[key]);
        })
    }
