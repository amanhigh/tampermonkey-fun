...
const watchListFlagSelector = ".uiMarker-2qpL8Ijj";
...
function filterSymbols(color, index) {
    ...
    let hasSelector = `${watchListLineSelector}:has(${colorSelector})`;
    let notSelector = `${watchListLineSelector}:not(:has(${colorSelector}))`;
...
function paintTVWatchList() {
    ...
    $(`${watchListNameSelector}[data-symbol-full*='BITFINEX'],[data-symbol-full*='BINANCE'],[data-symbol-full*='BITSTAMP'],[data-symbol-full*='KRAKEN'],[data-symbol-full*='FTX']`).find(watchListFlagSelector).css('color', 'green')
