...
const watchListChangeEvent = "watchListChangeEvent";
 ...
 */
function filterSymbols(filter, shift) {
...
function onWatchListChange() {
    waitOn(watchListChangeEvent,5,()=>{
        resetWatchList();
        paintTVWatchList();
        lastFilter && filterSymbols(lastFilter, false);
        paintTVScreener();
        paintName();
...
function watchSummary() {
    ...
    function addCount(indexCount, colorIndex) {
        buildLabel(indexCount.toString() + '|', colorList[colorIndex]).data('color', colorList[colorIndex]).appendTo($watchSummary)
            .mousedown((e) => {
                let filter = {color: $(e.target).data('color'), index: e.which, ctrl: e.originalEvent.ctrlKey};
                filterSymbols(filter, e.originalEvent.shiftKey);
            }).contextmenu(e => {
            e.preventDefault();
            e.stopPropagation();
        });
