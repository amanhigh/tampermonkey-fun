...
//Constants
const colorList = ['orange', 'red', 'dodgerblue', 'cyan', 'lime', 'greenyellow', 'brown', 'darkkhaki'];
...
function paintTVWatchList() {
    ...
    paint(watchListSymbolSelector, getCompositeTickers(), colorList[7]);
    //Paint Kite
...
function watchSummary() {
    ...
    function addCount(indexCount, colorIndex) {
        buildLabel(indexCount.toString() + '|', colorList[colorIndex]).data('color', colorList[colorIndex]).appendTo($watchSummary).mousedown((e) => {
    ...
    addCount(indexCount, 6);
    addCount(compositeCount, 7);
}
