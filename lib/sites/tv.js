 ...
 */
function filterWatchList(filter, shift) {
...
function onWatchListChange() {
    waitOn(watchListChangeEvent, 2, () => {
        resetWatchList();
        paintTVWatchList();
        lastFilter && filterWatchList(lastFilter, false);
        paintTVScreener();
        paintName();
    ...
    function addCount(indexCount, colorIndex) {
        buildLabel(indexCount.toString() + '|', colorList[colorIndex]).data('color', colorList[colorIndex]).appendTo($watchSummary)
            .mousedown((e) => {
                ...
                filterWatchList(filter, e.originalEvent.shiftKey);
            }).contextmenu(e => {
            e.preventDefault();
            e.stopPropagation();
        });
