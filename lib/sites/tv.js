...
const indexSymbols = ['CNXMIDCAP', 'CNXSMALLCAP', 'IXIC', 'DXY', 'NIFTY', 'NIFTYJR', 'US10Y', 'IN10Y', 'USOIL', 'USDINR', 'XAUUSD', 'XAGUSD', 'SHCOMP', 'BTCUSD', 'GOLDSILVER', 'BTC.D'];
const nameLength = 20
 ...
 * @param index
 */
function filterSymbols(color, index) {
    ...
    switch (index) {
        case 1:
            //Left Mouse: Show Only This Color (Name Filtering)
            $(watchListLineSelector).show();
            $(`${watchListLineSelector}:not(:has(span.${watchListSymbolSelector}[style*='color: ${color}']))`).hide();
            break;
        case 2:
            //Middle Mouse: Hide Only This Color
            $(`${watchListLineSelector}:has(span.${watchListSymbolSelector}[style*='color: ${color}'])`).hide();
            break;
        case 3:
            //Right Mouse: Show Only This Color (Flag Filtering)
            $(watchListLineSelector).show();
            $(`${watchListLineSelector}:not(:has(span.${watchListFlagSelector}[style*='color: ${color}']))`).hide();
            break;
        default:
...
function paintTVWatchList() {
    ...
    //Mark Cryptos //TODO:Improve Crypto Exchange Mgmnt
    $(`${watchListNameSelector}[data-symbol-full*='BITFINEX'],[data-symbol-full*='BINANCE'],[data-symbol-full*='BITSTAMP'],[data-symbol-full*='KRAKEN'],[data-symbol-full*='FTX']`).find(watchListFlagSelector).css('color', colorList[4])
...
function watchSummary() {
    ...
    function addCount(indexCount, colorIndex) {
        buildLabel(indexCount.toString() + '|', colorList[colorIndex]).data('color', colorList[colorIndex]).appendTo($watchSummary)
            .mousedown((e) => {
                filterSymbols($(e.target).data('color'), e.which);
            }).contextmenu(e => {
            e.preventDefault();
            e.stopPropagation();
        });
