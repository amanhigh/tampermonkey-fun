...
function logMissingFno() {
    ...
    message(`FNOS --> Total: ${fnoSymbols.length}, Missing: ${missingFnos.length}`)
...
function getWatchListSelected() {
    return $(`.selected-1d4hA4fK ${watchListSymbolSelector}`).toArray().map(s => s.innerHTML);
}
...
function getScreenerSelected() {
    return $(`.tv-screener-table__result-row--selected ${screenerSymbolSelector}`).toArray().map(s => s.innerHTML);
}
    ...
    switch (filter.index) {
        ...
        case 3:
            if (filter.shift) {
                ...
                $(`${screenerLineSelector}:has(${watchListFlagSelector}[style*='color: ${filter.color}'])`).hide();
            } else {
                $(`${watchListLineSelector}:has(${watchListFlagSelector}[style*='color: ${filter.color}']):hidden`).show();
                $(`${screenerLineSelector}:has(${watchListFlagSelector}[style*='color: ${filter.color}']):hidden`).show();
            }
            break;
        default:
...
function paintName() {
    ...
    //Flag Marking
    let m = GM_getValue(flagInfoStore, {});
    let $flag = $(`${watchListFlagSelector} > svg > path:nth(0)`);
    $flag.css('color', colorList[5]);
    colorList.forEach((c, i) => {
        if ((m[i] || []).includes(ticker)) {
            $flag.css('color', c)
        }
    })
}
...
function paintTickers(selector) {
    ...
    let flags = GM_getValue(flagInfoStore, {});

    for (let i = 0; i < colorList.length; i++) {
        paint(selector, orders[i] || [], colorList[i]);
        paintFlag(selector, flags[i] || [], colorList[i]);
    }
}

function paint(selector, symbols, colour, force = false) {
    paintCss(selector, symbols, {'color': colour}, force);
}

function paintFlag(selector, symbols, colour, force = false) {
    $(`${selector}`).filter((i, e) => force || symbols.includes(e.innerHTML))
        .parent().parent().parent().find(watchListFlagSelector).css('color', colour);
}

function paintCss(selector, symbols, css, force = false) {
    $(`${selector}`).filter((i, e) => force || symbols.includes(e.innerHTML)).css(css);
}

function resetColors(selector) {
    paint(selector, [], colorList[5], true);
    paintFlag(selector, [], colorList[5], true);
}
...
function toggleFlagSet(colorIndex) {
    let m = GM_getValue(flagInfoStore, {});
    let selected = getSelection();

    //Init Array if Required
    let set = new Set(m[colorIndex] || []);

    //TODO:Merge with OrderSet on Common logic ?
    //Toggle Selected Items
    selected.forEach((s) => set.has(s) ? set.delete(s) : set.add(s));

    //Save Back
    m[colorIndex] = Array.from(set);
    GM_setValue(flagInfoStore, m);
