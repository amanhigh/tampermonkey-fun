...
function removeFromWatchList() {
    ...
    message(`WatchList Del: ${removeTicker}`.fontcolor('red'), 5000)
...
function alertSummary(alertData) {
    ...
    if (orders) {
        orders.forEach((order) => {
            ...
            buildButton("", `${shortType}-${order.qty}`.fontcolor('yellow'), onDeleteGtt).data('order-id', order.id).appendTo($altz);
        })

    }

    // Add Alert Buttons
    message(`Altz Refresh: ${alertData.name}`.fontcolor('skyblue'))
    if (alrts) {
        alrts.sort(((a, b) => {
