//Selectors
const tickerSelector = '#header-toolbar-symbol-search > div';
...
function getWatchListTickers(visible = false) {
    let selector
    if (visible)
        selector = watchListSymbolSelector + ":visible";
    else
        selector = watchListSymbolSelector;

    return $(selector).toArray().map(s => s.innerHTML);
}
...
function navigateTickers(step) {
    //Next visible
    let visibleTickers = getWatchListTickers(true);
...
function openTicker(ticker) {
    //TODO:Remove Non Search Box if New Works
    ...
    waitInput('input.search-1t76YXcC', ticker);
}
...
function style(index) {
    // Template Selector
    waitClick('div.floating-toolbar-react-widgets__button')

    //Clicks Style based on Index.
    waitJClick(`tr.item-stVdeCwG:nth(${index})`)
}
