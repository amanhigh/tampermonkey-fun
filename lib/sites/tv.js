...
function removeFromWatchList() {
    let current = $('div[data-active=true]');
    let currentTicker = current.attr('data-symbol-short');
    let ticker = getTicker();
    if (currentTicker === ticker) {
        //Remove Previous Symbol. Incase of Last Symbol Prev is Nil
        current.parent().prev().find('.removeButton-2DU0hPm3').click();
    } else {
        message(`Skipping Remove, ticker Mismatch: ${ticker} != ${currentTicker}`.fontcolor('red'))
    }
}
...
function setGtt() {
    ...
    if (prices === "..") {
        ...
        GM_setValue(gttRequest, {symb: symb, qty: -1});
...
function getTriggers(alrts) {
    ...
    return {name: alrts.data.pairData.name, triggers: triggers};
...
function alertSummary(alertData) {
    ...
    message(`Alerts for: ${alertData.name}`)
    if (alrts) {
        alrts.sort(((a, b) => {
