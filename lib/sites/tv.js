...
function addToFlagSet(colorIndex) {
    let m = GM_getValue(flagInfoStore, {});
    let watchListSelected = $('.selected-1d4hA4fK').data('symbol-short').toArray();
    //Init Array if Required
    m[colorIndex] = m[colorIndex] || [];
    m[colorIndex].push(watchListSelected);
    console.log(m);
}
