...
const screenerSelector = ".tv-data-table__tbody";
const screenerHeaderSelector = ".tv-screener-table__th";
...
const indexSymbols = ['CNXMIDCAP', 'CNXSMALLCAP', 'IXIC', 'DXY', 'NIFTY', 'NIFTYJR', 'US10Y', 'IN10Y', 'USOIL', 'USDINR', 'XAUUSD', 'XAGUSD', 'SHCOMP', 'GOLDSILVER', 'BTC.D'];
const nameLength = 20
 ...
 * @param index
 */
function filterSymbols(color, index, ctrl, shift) {
    // console.log('Toggling Symbols', color, index);
    lastFilter = {color: color, index: index};
    ...
    switch (index) {
        case 1:
            if (ctrl) {
                $(`${watchListLineSelector}:has(${watchListFlagSelector}[style*='color: ${color}']):hidden`).show();
            } else {
                $(`${watchListLineSelector}:has(${watchListSymbolSelector}[style*='color: ${color}']):hidden`).show();
            }
            break;
        case 2:
            //Middle Mouse: TODO Add Functionality
            break;
        case 3:
            if (ctrl) {
                $(`${watchListLineSelector}:has(${watchListFlagSelector}[style*='color: ${color}'])`).hide();
            } else {
                $(`${watchListLineSelector}:has(${watchListSymbolSelector}[style*='color: ${color}'])`).hide();
            }
            break;
        default:
...
function paintAll() {
    paintTVWatchList();
    lastFilter && filterSymbols(lastFilter.color, lastFilter.index, false, false);
    paintTVScreener();
    paintName();
}
...
function paintTVWatchList() {
    ...
    //Mark Stocks
    $(`${watchListNameSelector}`).find(watchListFlagSelector).css('color', colorList[4]);
    //Mark Cryptos //TODO:Improve Crypto Exchange Mgmnt & Flag Painting
...
function paint(selector, symbols, colour) {
    $(`${selector}`).filter((i, e) => symbols.includes(e.innerHTML)).css('color', colour);
}
...
function watchSummary() {
    ...
    function addCount(indexCount, colorIndex) {
        buildLabel(indexCount.toString() + '|', colorList[colorIndex]).data('color', colorList[colorIndex]).appendTo($watchSummary)
            .mousedown((e) => {
                filterSymbols($(e.target).data('color'), e.which, e.originalEvent.ctrlKey, e.originalEvent.shiftKey);
            }).contextmenu(e => {
            e.preventDefault();
            e.stopPropagation();
        });
