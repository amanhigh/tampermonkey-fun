...
function setupFastAlertUI() {
    buildArea(areaId).appendTo('body');
    buildCheckBox(swiftId, false).appendTo(`#${areaId}`).change(fixTitle);
    buildLabel("", summaryId).appendTo(`#${areaId}`);
    buildInput(symbolId).appendTo(`#${areaId}`);
    buildInput(priceId).appendTo(`#${areaId}`).keypress((e) => {
        if (e.keyCode === 13) {
            setAlert();
        }
    });
