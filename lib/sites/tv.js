...
function addAlert(pairId, price) {
    const alerts = alertMap.get(pairId).push(new Alert(pairId, price));
    alertMap.set(alert.PairId, alerts);
...
function removeAlert(pairId, alertId) {
    const alerts = alertMap.get(pairId).filter(item => item.Id === alertId);
...
function resetAlerts() {
    ...
    searchSymbol(getMappedTicker(), function (top) {
        ...
        //Delete All Alerts
        let alertData = alertMap.get(top.pairId);
