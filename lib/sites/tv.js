...
const timeframeSelector = `#header-toolbar-intervals > div[class*=group] > div`;
...
//Sidebar
const deleteArrowSelector = 'div [data-name="removeAllDrawingTools"] div[class^=arrow]';
...
function createCard() {
    ...
    if (orderSet.get(2).has(ticker)) {
        ...
        type = "set"
    } else if (orderSet.get(0).has(ticker) || orderSet.get(1).has(ticker)) {
        type = "taken"
    } else {
...
function timeframe(timeFrameIndex) {
    ...
    //In SCALP Mode Fix Style Index
    styleIndex = scalpModeOn ? 0 : timeFrameIndex;
}
