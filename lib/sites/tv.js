...
const demandMenuScalp = ['TDZ', 'IDZ', 'VHDZ', 'HDZ'];
const supplyMenuScalp = ['TSZ', 'ISZ', 'VHSZ', 'HSZ'];
...
function getKiteTicker() {
    return getTicker().replace("_", "-");
}
...
function createCard() {
    ...
    //Not Taken Settings
    let type = "rejected";
    let listId = "5cfb58544c0c65863097655f";

    if (orderSet.get(2).has(ticker)) {
        //Taken Card Settings
        listId = "5b0fb8122611996bf3b1eb3b";
        type = "set"
    } else if (orderSet.get(0).has(ticker) || orderSet.get(1).has(ticker) || orderSet.get(4).has(ticker)) {
        type = "result"
    } else {
        //Switch Off SwiftKeys for Easy Typing.
        toggleSwiftKeys(false);
        const reason = prompt('Enter Reason', '');
        if (reason) {
            type = type + "." + reason
        }
        //Switch Back on for AHK to Screenshot.
    ...
    let context = ticker + "." + this.id + "." + type
    GM_setClipboard(context);
    message(`Loaded: ${context}`.fontcolor('green'))
}
...
function timeframe(timeFrameIndex) {
    ...
    //Based on Selected Timeframe Set Style Index to Be Used During Styling.
    styleIndex = timeFrameIndex;
}

function getTimeFrameIndex(styleIndex) {
    //Disable Hourly in Replay
    if (isReplayActive() && styleIndex == 3) {
        styleIndex--;
        message(`In Replay-Mode, Can't Switch Timeframe`.fontcolor('red'));
    }
    var styleName = getExchange() === NSE_EXCHANGE ? NSE_EXCHANGE : "DEFAULT";
    styleName = scalpModeOn ? "SCALP" : styleName;
    // console.log(styleName, styleIndex, ,timeFrameBar[styleName])
...
/**
 * Based on Enum Select Right Style by Selecting Correct Positions.
 * @param styleType
 */
function timeframeStyle(styleType) {
    if (styleType == "DZ")
        positions = scalpModeOn ? demandMenuScalp : demandMenu
    else
        positions = scalpModeOn ? supplyMenuScalp : supplyMenu
...
function trelloSectionUI() {
    ...
    buildWrapper('aman-trello-type').appendTo(`#${trelloId}`)
        ...
        .append(buildButton('wdh.trend', "W-T", createCard).data('id', '5e3712cfb57c1210b4627055'))
        .append(buildButton('wdh.ctrend', "W-CT", createCard).data('id', '5e3712df7f1630869f9d559d'));
}