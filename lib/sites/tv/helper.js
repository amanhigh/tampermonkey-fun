...
function pinExchangeTicker(tvTicker, exchange) {
    // TODO: #A Merge Pin Exchange with Pin Investing
 ...
 * @param {string} tvTicker - The TV ticker to cache
 * @param {PairInfo} pairInfo - The pair data to cache
 * @returns {void}
 */
function pinInvestingPair(tvTicker, pairInfo) {
    let pairSilo = GM_getValue('pairSilo', {});
    pairSilo[tvTicker] = new PairInfo(pairInfo.name, pairInfo.pairId);
    GM_setValue('pairSilo', pairSilo);
    message(`Pinned Investing Pair: ${tvTicker} -> ${pairInfo.name} (ID: ${pairInfo.pairId})`.fontcolor('yellow'));
}

/**
 * Map investing pair info from the pair silo cache
 * @param {string} tvTicker - The TV ticker to retrieve
 * @returns {PairInfo|null} - The cached pair data or null if not found
 */
function mapInvestingPair(tvTicker) {
    let pairSilo = GM_getValue('pairSilo', {});
    if (pairSilo[tvTicker]) {
        return new PairInfo(pairSilo[tvTicker].name, pairSilo[tvTicker].pairId);
    }
    return null;
}
...
function pinInvestingTicker(tvTicker, investingTicker) {
    ...
    message(`Pinned Ticker (TV->Investing): ${tvTicker} -> ${investingTicker}`.fontcolor('yellow'));
}
...
function processTextAction() {
    ...
    const actionHandlers = {
        ...
        P: () => pinInvestingTickerWithExchange(tvTicker, exchange, value),
...
function pinInvestingTickerWithExchange(tvTicker, exchange, investingTicker) {
    if (!exchange || !investingTicker) {
        displayHelpMessage();
        return;
    }
    pinInvestingTicker(tvTicker, investingTicker);
    searchSymbol(investingTicker, () => { }, exchange);
}
...
/**
 * @param {string} ltpSelector - The selector for the last traded price element.
 * @returns {number} - The last traded price as a float.
 */
function getLastTradedPrice(ltpSelector) {
    // Get the text from the selector and remove any commas
    const ltpText = $(ltpSelector).text().replace(/,/g, '');
    // Parse the cleaned text to a float
    return parseFloat(ltpText);
}
