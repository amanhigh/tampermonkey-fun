...
/**
 * A function that maps a TV ticker to a sequence from the dataSilo, 
 * returning the sequence if found, otherwise returning the DEFAULT_SEQUENCE.
 *
 * @param {type} tvTicker - the TV ticker to map to a sequence
 * @return {type} the mapped sequence or DEFAULT_SEQUENCE if not found
 */
function mapSequence(tvTicker) {
    let sequence = dataSilo.sequenceMap[tvTicker];
    if (sequence) {
        return sequence;
    } else {
...
function toggleSequence(tvTicker) {
    ...
    if (dataSilo.sequenceMap[tvTicker]) {
        ...
        //Map to High Sequence
        dataSilo.sequenceMap[tvTicker] = HIGH_SEQUENCE;
...
function displaySequence() {
    $(`#${inputId}`).val(mapSequence(getTicker()));
}
