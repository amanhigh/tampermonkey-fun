...
function mapTicker(tvTicker) {
    ...
    let investingTicker = dataSilo.getInvestingTicker(tvTicker);
    if (investingTicker) {
...
function buildReverseTickerMap() {
    reverseTickerMapInMem = {};
    for (let key in dataSilo.tickerMap) {
        reverseTickerMapInMem[dataSilo.tickerMap[key]] = key;
    }
}
...
function reverseMapTicker(investingTicker) {
    let tvTicker = reverseTickerMapInMem[investingTicker];
    if (tvTicker) {
        return tvTicker;
    }
...
function getSequence() {
    let sequence = dataSilo.getSequence(getTicker(), null);
    if (!sequence) {
        sequence = getDefaultSequence(getExchange());
    }
    return sequence;
}
...
function mapExchangeTicker(tvTicker) {
    return dataSilo.getExchangeTicker(tvTicker);
}
...
function pinExchangeTicker(tvTicker, exchange) {
    dataSilo.pinExchange(tvTicker, exchange);
...
function pinInvestingTicker(tvTicker, investingTicker) {
    dataSilo.pinInvestingTicker(tvTicker, investingTicker);
...
function loadDataSilo() {
    ...
    recentTickersInMem = new Set(dataSilo.recentTickers);
    //Enable/Disable Recent based on weather previous state had recent Tickers
    $(`#${recentId}`).prop('checked', recentTickersInMem.size > 0)
}

function saveDataSilo() {
    dataSilo.recentTickers = [...recentTickersInMem];
    dataSilo.save();
