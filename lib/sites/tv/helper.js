...
function toggleSequence(tvTicker) {
    ...
    let sequence = getExchange() === NSE_EXCHANGE ? DEFAULT_SEQUENCE : HIGH_SEQUENCE;

    //Remove Mapping If Exists.
    if (dataSilo.sequenceMap[tvTicker]) {
        dataSilo.sequenceMap.delete(tvTicker);
    } else {
        // Flip sequence
        sequence = sequence === HIGH_SEQUENCE ? DEFAULT_SEQUENCE : HIGH_SEQUENCE;
...
function getStyleIndex(timeFrameIndex) {
    ...
    // HACK: Modify to Freeze Sequence
