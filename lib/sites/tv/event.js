 ...
 * 
 * @param {string} zoneType - The type of zone to apply, either "DZ" (Demand Zone) or "SZ" (Supply Zone)
 */
function SelectTimeFrameStyle(zoneType) {
    // Get the style corresponding to the current timeframe
    const style = timeFrame.style;
    
    // Combine the timeframe-specific style with the zone type and apply it
    Style(style + zoneType);
}
