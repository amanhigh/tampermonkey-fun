...
function HandleGttOrderButton() {
    ...
    message(`${order.symb} (${order.ltp}), Qty ${order.qty}, SL:ENT:TP: ${order.sl} - ${order.ent} - ${order.tp}`.fontcolor('yellow'));
...
function readOrderPanel() {
    ...
    // Confirm Qty
    qty = prompt(`Qty (2X): ${qty} (${doubleQty}) ----------- Risk: ${risk} (${orderRiskLimit})`, qty);
...
function RecordJournal() {
    ...
    ReasonPrompt(function (reason) {
        ...
        // Augment Reason
        if (reason === 'MISS') {
            // Modify Trade Type when Reason is Miss
            type = "miss"
