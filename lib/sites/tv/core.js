...
/* global TimeFrame, AlertStore, message, buildArea, buildWrapper, buildCheckBox,
   buildButton, buildInput, EnableSwiftKey, HandleSequenceSwitch, HandleRefreshButton,
   HandleAlertButton, HandleJournalButton, onRecentTickerReset, HandleInputSubmit,
   HandlePriceSubmit, HandleAlertContextMenu, RecordJournal, fnoSymbols,
   areaId, topId, swiftId, seqId, refreshBtnId, altCreateBtnId, journalBtnId,
   recentId, summaryId, midId, displayId, inputId, alertsId, ordersId, journalId,
   auditId */

// Export functions that are used in other files
window.LogMissingData = LogMissingData;
window.SetupTvUI = SetupTvUI;
window.LogMissingFno = LogMissingFno;

// Declare global variables that are used across files
window.freezeSequence = undefined;
window.filterChain = [];
window.timeFrame = TimeFrame.DAILY;

// In Memory Stores
window.alertStore = new AlertStore();

// Saved Silo
window.dataSilo = undefined;
window.pairSilo = undefined;
window.orderSet = undefined;
window.flagSet = undefined;

//Replay
window.replayCron = false;
window.runReplay = false;
...
function LogMissingFno() {
    let missingFnos = [...fnoSymbols].filter((sym) => {
        for (let i of [0, 1, 4, 6]) {
            if (window.flagSet.get(i).has(sym)) {
