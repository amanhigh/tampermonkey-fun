...
class AbstractSet {
    ...
    containsAny(ticker) {
        return this.setMap.forEach((v, i) => {
            if (v.has(ticker)) {
                return true;
            }
        });
...
/**
 * Manages audit results for multiple tickers
 */
class AlertAuditMap {
    results
    ...
    addResult(ticker, state) {
        this.results.set(ticker, new AlertAuditResult(ticker, state));
    }

    getResult(ticker) {
        return this.results.get(ticker);
    }
    ...
    getFilteredResults(state) {
        return this.getAllResults().filter(result => result.state === state);
    }
