...
function CreateTickerAlert(ticker, price) {
    searchSymbol(ticker, function (top) {
        message(`<span style="color: LightSalmon;">Alert: ${top.name} = ${price}</span>`);
        createAlert(top.pairId, price, onAlertCreate);
    });
}
...
function HandleAlertClick(evt) {
    if (evt.Name) {
        ...
        pinAlertName(getTicker(), evt.Name);
    } else {
        // Open Ticker
        OpenTicker(evt.Ticker);
    }
}