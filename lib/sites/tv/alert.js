...
/**
 * Audit alerts for all mapped tickers
 * @returns {AlertAuditMap} Map of tickers to their alert status
 */
function auditAlerts() {
    const auditMap = new AlertAuditMap();
    const tickers = getAllAlertTickers();

    tickers.forEach(ticker => {

        // Get pair info for the ticker
        let pairInfo = mapInvestingPair(ticker);
        if (!pairInfo) {
            auditMap.addResult(ticker, AlertState.NO_ALERTS);
            return;
        }

        // Get alerts using existing alertMap
        ...
        if (alerts.length === 0) {
            ...
            auditMap.addResult(ticker, AlertState.SINGLE_ALERT);
        } else {
            auditMap.addResult(ticker, AlertState.VALID);
        }
    });
...
/**
 * Shows a popup to audit alerts
 * @param {number} [batchSize=5] Number of alerts to show at once
 */
function ShowAuditAlertPopup(batchSize = 5) {
    const auditMap = auditAlerts();
    const singleAlerts = auditMap.getFilteredResults(AlertState.SINGLE_ALERT);

    if (singleAlerts.length === 0) {
        message("No single alerts found", "yellow");
        return;
    }

    // Take first batch of alerts
    const currentBatch = singleAlerts.slice(0, batchSize);

    // Create reasons array for SmartPrompt (just tickers)
    const reasons = currentBatch.map(result => result.ticker);

    // Show popup with just the tickers, no additional actions
    SmartPrompt(reasons)
        .then(response => {
