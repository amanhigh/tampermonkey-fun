...
function onAlertForceRefresh(data) {
    ...
    // Perform Audit
    auditMap = auditAlerts(); //BUG: Screen Blocking
    DoAlertAudit();
...
function auditAlerts() {
    ...
    tickers.forEach(ticker => {
        // Skip if ticker is in any orderSet list
        if (orderSet.containsAny(ticker)) {
            return;
        }
 ...
 */
function DoAlertAudit(batchSize = 5) {
    const singleAlerts = auditMap.getFilteredResults(AlertState.SINGLE_ALERT);
    const noAlerts = auditMap.getFilteredResults(AlertState.NO_ALERTS);
