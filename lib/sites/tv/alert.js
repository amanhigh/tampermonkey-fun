...
function loadCachedAlert() {
    ...
    if (!orderSet.contains(7, symb)) {
        ...
        if (pairInfo) {
            alertSummary(alertMapInMem.get(pairInfo.pairId));
        } else {
...
function onAlertForceRefresh(data) {
    ...
    auditAlerts((auditResults) => {
        auditMapInMem = auditResults; // Update global auditMap
        DoAlertAudit();
    });
...
function saveAlerts(data) {
    ...
    $('<div/>').html(data).find('.js-alert-item[data-trigger=price]').each((i, alertText) => {
        ...
        let alerts = alertMapInMem.get(alert.PairId) || [];
        //Add Alert
        count++;
        alerts.push(alert);
        //Update Map
        alertMapInMem.set(alert.PairId, alerts);
    });
    return count;
}
...
function removeAlertStore(pairId, alertId) {
    let alerts = alertMapInMem.get(pairId) || [];
    alerts = alerts.filter(item => item.Id != alertId);
    alertMapInMem.set(pairId, alerts);
}
...
function auditAlerts(callback) {
    ...
    function processBatch() {
        ...
        for (let i = currentIndex; i < endIndex; i++) {
            ...
            auditMapInMem.addResult(ticker, state);
        }
        ...
        if (currentIndex < tickers.length) {
            ...
            message(`Audited ${auditMapInMem.results.size} tickers`, 'green');
            if (callback) {
                callback(auditMapInMem);
            }
        }
    }

    requestAnimationFrame(processBatch);
    return auditMapInMem;
}
...
function auditTickerAlerts(ticker) {
    ...
    const alerts = alertMapInMem.get(pairInfo.pairId) || [];
...
function DoAlertAudit(batchSize = 10) {
    ...
    const singleAlerts = filterResults(auditMapInMem.getFilteredResults(AlertState.SINGLE_ALERT));
    const noAlerts = filterResults(auditMapInMem.getFilteredResults(AlertState.NO_ALERTS));
...
function refreshAuditButton() {
    ...
    const result = auditMapInMem.getResult(investingTicker);
