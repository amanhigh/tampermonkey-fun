...
function loadCachedAlert() {
    ...
    if (!orderSet.get(7).has(symb)) {
        ...
        if (pairInfo) {
            ...
            message(`Can't load alerts. No pair info found for symbol: ${symb}`.fontcolor('red'));
        }
    }
}
 ...
 * @return {void} 
 */
function CreateTickerAlert(ticker, price) {
    let pairInfo = mapInvestingPair(ticker);
    if (pairInfo) {
        createAlert(pairInfo.name, pairInfo.pairId, price, onAlertCreate);
    } else {
        message(`Can't create alert. No pair info found for symbol: ${ticker}`.fontcolor('red'));
    }
}
 ...
 *
 * @param {string} tvTicker - The TradingView ticker symbol
 ...
 */
function mapAlert(tvTicker, exchange = "", searchTerm = "") {
    const symbolToSearch = searchTerm || tvTicker;
    message(`Searching for ${symbolToSearch} on ${exchange}`.fontcolor('yellow'));
    return fetchSymbolData(symbolToSearch, exchange)
        .then(pairInfo => {
            pinInvestingPair(tvTicker, pairInfo);
            return pairInfo;
        })
        .catch(error => {
            message(`Error mapping alert: ${error.message}`.fontcolor('red'));
            throw error;
        });
}