...
function auditAlerts(callback) {
    ...
    function processBatch() {
        ...
        for (let i = currentIndex; i < endIndex; i++) {
            ...
            if (!pairInfoCache[ticker]) {
                pairInfoCache[ticker] = mapInvestingPair(ticker) || null;
            }

            const pairInfo = pairInfoCache[ticker];
            if (!pairInfo) {
            ...
            const state = alerts.length === 0 ? AlertState.NO_ALERTS :
